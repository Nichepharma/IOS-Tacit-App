...
+(void)setAnimationsWithObject : (id) item withType :(NSString *)methodName animationWithDelay :(float)__Du {
    ...
    if ([methodName isEqualToString:@"setFadeAnimations"]) {
    ...
    } else if ([methodName isEqualToString:@"setFadeOutAnimations"]) {
            [item setFadeOutAnimations:__Du];
        ...
        [item setWidthAnimations:__Du];
    } else if ([methodName isEqualToString:@"setWidthBarAnimations"]){
        [item setWidthBarAnimations:__Du];
    }
}
}

+(void)setAnimationsWithObjectChangeImages  : (id) item
                                 toNewImage :(UIImage *)new_img2 withDelay : (float) __DU{
...
+(void)setAnimationsWithObjectChangeImagesWithoutAlpha : (id) item  toNewImage :(UIImage *)new_img2 withDelay : (float) __DU{
    [item changeImageWithAnimationsToNewImageWithoutFade:new_img2 withDelay:__DU];
}
